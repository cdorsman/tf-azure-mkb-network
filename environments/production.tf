...
data "azurerm_client_config" "current" {}
  ...
  locals {
    ...
    location            = "West US 2"
    vnet_name           = "mkb-vnet"  # VM Scale Set configuration
...
module "vmss" {
  ...
  vmss_name          = "mkb-vmss"
  public_ip_name     = "mkb-vmss-public-ip"
  load_balancer_name = "mkb-vmss-lb"
  vm_sku             = local.vm_sku
...
module "monitoring" {
  ...
  log_analytics_workspace_name = "LogWorkspace"
  action_group_name            = "mkb-alert-group"
  application_insights_name    = "mkb-app-insights"
...
module "dashboard" {
  ...
  dashboard_name = "MKB-Monitoring-Dashboard"
...
module "bastion" {
  ...
  bastion_name           = "mkb-bastion"
  bastion_public_ip_name = "mkb-bastion-public-ip"
